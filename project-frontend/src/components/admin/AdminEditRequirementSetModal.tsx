...
type AdminEditRequirementSetModalProps = {
  onClose: () => void;
  requirementSetId: number;
};

export default function AdminEditRequirementSetModal({
  onClose,
  requirementSetId,
}: AdminEditRequirementSetModalProps) {
  ...
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      try {
        const res = await api.get(`/RequirementSet/${requirementSetId}`);
        setSetName(res.data.title);
        setDeadline(res.data.deadline?.split("T")[0] || "");
        setRequirements(
          res.data.requirements?.map((r: Requirement) => r.title) || []
        );
      } catch (err) {
        toast.error("Failed to fetch requirement set.");
        console.error("Failed to fetch requirement set.", err);
      } finally {
        setIsLoading(false);
      }
    })();
  ...
  const handleSubmit = async () => {
    ...
    try {
      setIsLoading(true);
      await api.put(`/RequirementSet/${requirementSetId}`, {
        title: setName,
        deadline,
        requirements,
      });

      toast.success("Requirement set successfully updated.");
      onClose();
      window.location.reload();
    } catch (err) {
      console.error(err);
      toast.error("Failed to update requirement set.");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="flex flex-col w-full md:w-1/2 p-4 sm:p-8 bg-gray-100 md:flex-1 md:overflow-auto">
          ...
          <div className="flex flex-row w-full gap-4 self-justify-end justify-end">
            ...
            <button
              ...
              className="flex flex-row p-2 text-gray-50 cursor-pointer rounded-lg items-center gap-2 bg-yellow-500 hover:bg-yellow-600"
            >
              <FaSave /> Update Set
            </button>
          </div>
